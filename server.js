...
// --- API ---
app.get('/health',(_,res)=>res.json({ok:true,uptime:process.uptime(),github:!!octokit}));
...
app.post('/api/github/write',async(req,res)=>{ try{ const {path,code,sha,message}=req.body; // auto-backup
  if(path==='server.js'){ try{ const old=await ghGet(path); await ghPut(`backups/${Date.now()}-server.js`,old.code,null,'backup'); }catch{} }
  await ghPut(path,code,sha,message); res.json({ok:true,deploying:true});
